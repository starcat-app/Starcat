...
| `codex/agent-runtime-trace` | 本地；`../Starcat-agent-runtime-trace` worktree | 基于 `dev@f033e4d9` 把 Codex、DeepSeek Harness 与 Built-in Loop 的真实运行事件持久化为动态 Trace，并重构 Agent Run Surface 时间线。 | `开发中`；独立 worktree 已创建，正在实现事件契约、迁移、三 Runtime 映射、UI 与真实 E2E。 | 完成专项测试和真实 Codex E2E 后提交，等待 dong4j 验收并授权合回 `dev`。 |
