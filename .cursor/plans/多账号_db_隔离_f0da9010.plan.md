...
**本次只做的事**：在 [docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md](docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md) 末尾追加"多账号下的 CloudKit 设计预研"章节，把上述 X / Y / Z 三个方案与 trade-off 写清楚，dong4j 几周后接 CloudKit 时直接拾起。
...
- [docs/功能实现总览.md](docs/功能实现总览.md)：追加 D-XX 技术债条目（用户数据按账号隔离），完成后勾选 + `> 实现：...` 行
- [docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md](docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md)：末尾追加"多账号下的 CloudKit 设计预研"章节
- [docs/3-设计/详细设计/01-数据库设计.md](docs/3-设计/详细设计/01-数据库设计.md)：在文件路径章节追加"按 user 隔离"说明（如该文档存在）