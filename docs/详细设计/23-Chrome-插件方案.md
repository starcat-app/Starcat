...
完全采用原方案 §4.1 / §8.3 的思路，但**收敛到 5 个 action**，与 R-04 主表 + 4 详情页骨架对齐：
...
- README 翻译（V0.2，待 §12 Q2 拍板可提到 V0.1）
- Trending 页面增强（V0.2）
...
| V0.2 | F10：README 翻译触发（若 §12 Q2 在 V0.1 未做） | 复用 `ReadmeTranslationService` | 0.3 周 |
...
- `docs/详细设计/04-技术选型.md` — 浏览器侧技术栈是 Chrome MV3 + 原生 JS / TS，与 Starcat 主项目正交
...
2. dong4j 拍板施工后 → 实施过程中按 §13 里程碑勾选子项
3. 每个里程碑完成后 → 在 `功能实现总览.md` 对应条目下追加 `> 实现：...` 行
4. 第三方依赖（如 SwiftNIO）若引入 → 必须按 CLAUDE.md「开源致谢同步规则」追加 `AboutDependency` 条目

---

> **方案就绪状态**：本文档已覆盖背景、定位、能力边界、通信通道、MVP 范围、路线图、收益、风险、未决项、里程碑、文档关联十一个维度，足以作为 dong4j 拍板施工的依据。**待拍板事项见 §12 Q1~Q10**，建议优先就以下两项给方向：
>
> 1. **§6.2 反向链路是否走方案 A（本地 HTTP + Bearer）** —— 这是 M1 / M4 / M5 / M6 的地基
> 2. **§12 Q1 / Q2 / Q4 / Q5** —— 决定 MVP 范围与外观，影响 M3 / M4 工期估算
