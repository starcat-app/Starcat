...
| 显示条件 | `RepoCardViewData.isStarred == true` |
| 不显示场景 | Activity 卡片（kind icon 已暗示已 star，避免冗余） |
...
|---|---|---|---|---|---|
| Manage | 40pt | ✓ | 2 行 | Lang → Stars → Forks → Archive → starredAt | — |
...
enum RepoDetailAction: Identifiable {
    case share(handler: () -> Void)
    case ai(handler: () -> Void)
    case weeklyIssue(number: Int, url: URL)
    case custom(id: String, label: LocalizedStringKey, systemImage: String, handler: () -> Void)
    var id: String { ... }
}
```
