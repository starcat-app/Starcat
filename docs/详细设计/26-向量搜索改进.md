...
| 文件 | 职责 |
|---|---|
| [`Features/AI/ReadmePreprocessor.swift`](../Starcat/Features/AI/ReadmePreprocessor.swift) | README 清洗 + 截断（HTML / Markdown 两入口） |
...
| 触发源 | 行为 |
|---|---|
| 用户在 Settings 点"开始预拉" | `SemanticIndexBuilder.start()` 后台慢速拉全部缺 README 的 repo |
| AI 摘要生成成功 | `refreshIndexIfChanged(for: repo)` 单 repo 路径 |
| 用户笔记 upsert 成功（已带 800ms debounce） | `refreshIndexIfChanged(for: repo)` |
| 详情页 README 拉到（SWR 二阶段刷新） | 异步补 raw Markdown 落 `readmes.content` + 调 `refreshIndexIfChanged` |
| 搜索时（兜底，无 force） | `ensureIndexed` 内部仍走 diff 判定，缺失或超阈值才补建 |
...
| Suite | 覆盖 |
|---|---|
| `ReadmePreprocessorTests` | HTML 边界（img/script/style/Badge/HTML entities/超长截断） |
