...
- [ ] **隐私政策 HTTPS 托管** — 关于页已链 `https://starcat.app/privacy`，需确认页面真实可访问且与 App 实际数据行为一致
- [ ] **EULA / 用户协议** — 关于页已链 `https://starcat.app/eula`，同上
- [ ] **支持页 / 联系邮箱** — 关于页链 `https://starcat.app/support`、`mailto:support@starcat.app`
...
| **Starcat 客户端** | 默认 Bearer Token | 发版构建注入 | `Configs/Secrets.xcconfig` 的 `STARCAT_PRODUCTION_API_KEY` |
| **Starcat 客户端（覆盖）** | 用户自填 Key | App 内 **设置 → 服务** → 各服务 API Key | 持久化 Keychain，优先级高于 baked-in |

**对齐规则**：`STARCAT_PRODUCTION_API_KEY` 的值 **必须** 同时存在于四个 Fly App 的 `API_KEYS` 里（通常四端共用同一把 `sk-starcat-...` 即可）。
...
# 2. 把同一个 key 写入 supports/starcat-{sharing,trending,weekly,wiki}-api/.env 的 API_KEYS=
# 3. 推到 Fly（含 GITHUB_TOKENS、BASE_URL、STORE_FILE 等，见 supports/docs/fly-io-环境变量.md）
...
- [ ] **A. baked-in（推荐首版）**：存在 `Configs/Secrets.xcconfig` 且 `STARCAT_PRODUCTION_API_KEY` 非空 → 用户零配置可用 Trending / Weekly / Sharing / Wiki
...
### 3.2 Pro / 订阅策略（StoreKit 未接入）

`docs/PRO 订阅功能划分.md` §8 **D-1～D-8 均未拍板**；当前 **全功能对全员开放**，无付费墙。

- [ ] **D-1** CloudKit 多端同步归免费还是 Pro
- [ ] **D-2** AI 标签 / 摘要试用次数（3 vs 10 vs 0）
- [ ] **D-3** 免费版标签数量上限（20 vs 50 vs 不限）
- [ ] **D-4** BYOK 用户是否仍需 Pro
- [ ] **D-5** 自动 AI 整理：免费是否允许单次手动触发
- [ ] **D-6** 菜单栏 / Spotlight / Shortcuts / Widget 免费 vs Pro 边界
- [ ] **D-7** AI Discovery（Show HN）免费 vs Pro 边界
- [ ] **D-8** Chrome 插件免费 vs Pro 边界

| 首版策略 | 动作 |
|----------|------|
| **全功能免费公测** | 隐私政策 / 商店描述写清 AI 与数据使用；Pro 文档标 v0.2 |
| **首版即收费** | 先拍板 §8 八项 + StoreKit 2 + Quota + 付费墙 UI（大工程） |
...
- Chrome 插件 V0.1
- StoreKit / Quota / 菜单栏 / Spotlight / Widget
...
| `docs/PRO 订阅功能划分.md` | 免费 vs Pro 草案 + §8 待决策 |
