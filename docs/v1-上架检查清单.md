...
> 2026-06-23 追加：按 Apple 官方 Human Interface Guidelines / App Review Guidelines 复扫后，Starcat 主体 macOS 形态（原生三栏、系统 toolbar、Settings scene、popover / sheet、明暗主题）方向正确；上架前仍需收尾 **UI 可访问性、设置页控件、截图/Review Notes 披露** 三类问题，详见 §2.5。
...
> - [Apple App Review Guidelines](https://developer.apple.com/app-store/review/guidelines/) §2.1 / §2.3 / §4 / §4.8 / §5.1
...
|--------|------------|------------------|----------|
| **P0** | App Review 2.3：截图、描述、预览必须准确展示 App 核心体验；截图不能只是标题页 / 登录页。 | `docs/上架信息准备.md` §1.4 截图仍是占位；Starcat 首屏有 GitHub 登录，若只截登录页会不符合审核期望。 | 上架截图至少覆盖：主三栏、README 详情、AI 摘要/对话、智能集合、Activity/Weekly；截图内使用虚构或可公开测试数据，避免真实个人账号信息。 |
...
> Direct 渠道（2026-06-27）：官网 DMG + Lemon Squeezy 授权码方案已落档，见 **`docs/Direct分发与LemonSqueezy授权方案.md`**；App Store build 不展示授权码 / Lemon Squeezy / 外部购买入口。
...
| **官网 Direct 版** | ⏳ 单独构建 / 单独 UI / Lemon Squeezy license；不混入 App Store build |
...
- Sparkle 应用内自动更新（Direct 版官网 DMG 首期可先用手动下载，自动更新后续接入）
- Direct 分发授权码实现（Lemon Squeezy / License API / Direct 设置页）
- 结构化搜索页 / 保存搜索条件
...
- i18n 大批量收尾（`String.l10n`、相对时间 formatter；sheet / popover locale 复扫已通过，见 §2.5）
...
| `docs/Direct分发与LemonSqueezy授权方案.md` | 官网 DMG + Lemon Squeezy 授权码的 Direct 渠道方案 |
...
| 2026-06-23 | 追加 Apple 官方 HIG / App Review UI 走查 §2.5；把 2026-06-17 UI 小修完成口径更新为复扫后的上架前待整理项 |
| 2026-06-23 | About → Support 新增 StoreKit 系统评分入口；Sheet / Popover 环境继承复扫通过并清理 false positive |
| 2026-06-27 | 补充 Direct 渠道口径：官网 DMG 版走 Lemon Squeezy 授权码，App Store build 保持 StoreKit-only；链入 `docs/Direct分发与LemonSqueezy授权方案.md`。 |
