...
- [ ] **确认发版构建策略**（二选一，必须拍板）：
  - **A. baked-in**：`cp Configs/Secrets.xcconfig.template Configs/Secrets.xcconfig`，填入 `STARCAT_PRODUCTION_API_KEY`，发版机构建（见 template 注释 + `StarcatAPIKey.swift`）
  - **B. BYOK-only**：不发 baked-in key，首启 Trending / Weekly / Sharing / Wiki 会 401，需在 UI / 商店描述中明确引导用户到「设置 → 服务」填 Key
