...
>
> 状态：**草案 v1（2026-06-15）**，需要 dong4j 在第 §8「待决策项」打钩或修订后才能进入实施阶段。
>
> 前置阅读：`docs/功能清单.md` §7、`docs/需求分析.md` §7、`docs/概要设计.md` §4.1。
...
### ⚠️ D-1：CloudKit 多端同步归免费还是 Pro

**选项 A（推荐）**：归 **免费**

- 理由：① 用户数据云端备份是基础需求，不应付费；② Starcat 不靠"同步"卖钱，靠 AI；③ CloudKit 由 Apple 承担基础设施成本，对 Starcat 不构成持续负担；④ 让 iPhone / iPad 端能用基础同步会拉动多端覆盖率。
- 劣势：失去一个 Pro 卖点；竞品 OhMyStar 把同步放 Pro 是因为"同步"是他们的核心卖点。

**选项 B**：归 **Pro**

- 理由：① 与 Apple Notes / Bear / Things 一致都是 Pro；② 多设备同步对单设备用户无价值，可作为多设备用户的升级动力。
- 劣势：免费版价值缩水；用户跨设备体验不连贯。

**选项 C（折中）**：免费版**单向同步**（只读云端），Pro 版**双向同步**

- 理由：让单设备用户体验"备份"语义，多设备协作需 Pro。
- 劣势：实施复杂度高，CloudKit 单向同步技术上别扭。
...
> 当本文档通过 dong4j 拍板后的执行步骤。**禁止现在就动手实施**，仅作规划记录。
