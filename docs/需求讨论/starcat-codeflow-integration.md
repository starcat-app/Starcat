...
用户也可以在「设置 → 集成服务 → CodeFlow」中选择自定义输出目录。选择后，该目录本身就是 CodeFlow 输出根目录，不再额外拼接隐藏的容器路径。例如用户选择：
...
设置页展示当前输出目录的只读路径，并提供「选择目录」「在 Finder 中显示」「恢复默认」操作。切换输出目录后不自动迁移旧生成物，新目录按空数据重新扫描；项目尚未上线，不增加旧目录兼容和迁移逻辑。
...
在「设置 → 集成服务」增加 CodeFlow 输出目录与数据管理入口。
...
- **打开**：在默认浏览器打开现有 `index.html`；
- **预览 / 详情**：查看文件路径、大小、生成次数和生成器版本；
