...
- [ ] 增加 Awesome 来源列表、状态、计数、同步结果和错误展示。
- [ ] 增加来源新增、编辑、稳定排序以及 revision 冲突处理。
- [ ] 增加同步、发布、下架确认和 sync run 状态恢复。
- [ ] 验证 Admin key 不进入前端持久化或日志，且原有 Weekly/Discovery 操作无回归。

## 4. Starcat 数据层

- [ ] 追加实施时下一可用 `registerVN` migration，不修改 `v1-initial`。
- [ ] 增加 Awesome DTO、公共 API、ETag/304 和错误 envelope 解码。
- [ ] 增加账户隔离的来源目录、订阅、条目和首次配置状态缓存。
- [ ] 增加精选来源 SWR 刷新、单来源事务替换、失败保留旧条目和下架降级。
- [ ] 增加自定义来源输入归一化、公开 Repo/README 核验、本地 AST 解析和删除语义。
- [ ] 实现“全部 Awesome”按 `gh_repo_id` 去重、稳定排序和多来源证据聚合。

## 5. Starcat 界面

- [ ] Awesome 位于探索的周刊下方，未新增一级主栏目。
- [ ] 周刊显示语言列表；Awesome 显示全部及已启用来源，切换状态互不串扰。
- [ ] Awesome 名称右侧提供独立管理按钮，且不会误触整行选择。
- [ ] 首次点击自动打开卡片式来源 Sheet；取消不完成，完成零选择也不再自动弹出。
- [ ] 来源卡片展示图片回退、名称、介绍、仓库、项目数、推荐和勾选状态。
- [ ] 支持添加、停用和删除本地自定义来源，且不上传 Discovery。
- [ ] 中栏支持全部/单来源、章节、搜索与排序；外部链接不进入 Repo 列表。
- [ ] 右栏复用现有 Repo 详情并展示当前来源、其他来源和安全的原始 README 链接。
- [ ] 接入 i18n、Accessibility、键盘焦点以及 loading/error/empty/stale/unavailable 状态。
