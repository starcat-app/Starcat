...
- [x] 先新增并提交第一轮审查报告。
- [x] 核对展示层投影没有污染数据库、embedding、分页和普通统计。
- [x] 核对 Provider 复用、缓存键、保存、删除与 metadata 一致性。
- [x] 核对生成状态、取消传播、临时清理和 repo 切换竞态。
- [x] 核对 XML、私有仓库、CloudKit 与 Debug 隐私边界。
...
- [x] 先新增并提交第二轮审查报告。
- [x] 核对第二项顺序、独立统计、生成入口与锁定状态。
- [x] 核对编辑、校验失败不关闭、删除确认和重新生成闭环。
- [x] 核对下载未保存草稿、默认文件名和错误展示。
- [x] 核对进度阶段、hover / focus 停止、失败重试和生命周期取消。
- [x] 核对 UI 规范、accessibility、tooltip、sheet locale 和双语文案。
- [x] 报告发现的问题逐个修复并按小功能提交。
- [x] 修复后重新执行相关定向测试并回填报告。
...
- [x] 至少三轮审查完成后新增并提交清洁复审报告。
- [x] 清洁复审确认无遗留功能、测试、文档或一致性缺口。
- [x] 本 Checklist 全部可自动验证项回填。
- [x] 人工 UI 验收项明确列出，不伪造完成。
- [x] 新增并提交 `RepoContext分片管理结果报告.md`。
- [x] 结果报告列出实现、验证、提交、已知边界和工程总览待确认草案。
- [x] 确认没有 push。
