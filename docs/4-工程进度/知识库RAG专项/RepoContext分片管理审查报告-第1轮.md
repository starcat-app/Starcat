...
> 结论：发现的 1 个 P1、1 个 P2 均已修复，架构、数据与取消边界审查通过。
...
- 修复提交：`c63e0935 fix(rag): 修复 RepoContext 自动切仓取消竞态`。
- P1：搜索/筛选使当前仓库退出候选集时，程序化切仓会先取消旧 Task、调用 `.tmp` 窄清理并清空旧 XML，避免后台继续下载或新详情卡在旧进度。
- P2：新增 `RepoContextGenerationIdentity` 纯值边界，结果必须同时匹配 generation UUID 与 repo id；测试覆盖当前请求、请求过期、repo 过期及空身份。
- 修复后 `RepoContextStorageTests` 7 项通过，`Starcat` 随测试重新编译通过。
- 最终结论：本轮问题已清零，可以进入第 2 轮 UI、功能完整性与 i18n 审查。
