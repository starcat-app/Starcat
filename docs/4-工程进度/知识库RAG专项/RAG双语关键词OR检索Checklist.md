...
- [ ] 新增 RAG 专用关键词查询构造器，不修改普通 `FTSQuery.sanitize` 行为。
- [ ] 每个关键词和短语独立安全转义，模型文本不能注入 FTS5 语法。
- [ ] 多项使用显式 OR，任意一个关键词命中即可召回。
- [ ] 空关键词数组使用 `semanticQuery` 的有界 OR token 降级。
- [ ] 最终 FTS5 表达式可进入检索 Trace，但不记录分片正文。
- [ ] 补齐转义、OR、短语、去重、数量和降级单元测试。
...
- [ ] 先新增并提交第二轮审查报告。
- [ ] 审查 Keyword / Vector / Rerank 查询职责和独立降级。
- [ ] 审查漏斗、Plan、Debug、历史恢复展示的是否为真实执行值。
- [ ] 审查零命中、执行失败和已跳过是否正确区分。
- [ ] 对照实现审查测试是否存在缺口或仅测试了 helper。
- [ ] 报告发现的问题逐项修复并按小功能提交。
- [ ] 修复后重跑相关定向测试并回填报告。
...
- [ ] 至少三轮审查完成，最后一轮无遗留功能缺口。
- [ ] 本 Checklist 全部项目均有真实证据并已回填。
- [ ] 新增并提交 `RAG双语关键词OR检索结果报告.md`。
- [ ] 结果报告列出实现、验证、提交、已知边界和工程总览待确认内容。
- [ ] 确认任务分支没有 push。
- [ ] 确认 worktree 干净，所有改动均已提交。
