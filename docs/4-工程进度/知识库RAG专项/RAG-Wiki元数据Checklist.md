...
- [ ] 详情页改用 `WikiContextService`，不再直接请求 Wiki API。
- [ ] 全局搜索详情改用 `WikiContextService`，不再直接请求 Wiki API。
- [ ] Repo AI Chat 与 Companion 使用相同 cache-first 入口。
- [ ] fresh 只读缓存，stale 先返回旧值再刷新，miss 先返回空再入队。
- [ ] 只保留 indexed 且为 `http` / `https` 的 DeepWiki、ZRead、CodeWiki 链接。
- [ ] 私有仓库不向外部 Wiki 服务发送 identity。
- [ ] 所有成功刷新结果写入 `DiskWikiCache`。
- [ ] 补齐统一读取、TTL、URL 过滤和私有仓库测试。

## 3. cache-first 后台补齐

- [ ] 新增 Wiki 后台补齐协调器与小并发队列。
- [ ] 排队与执行期间按 repo identity 去重。
- [ ] App 当前数据库就绪后扫描已有知识库仓库。
- [ ] 新仓库加入知识库后自动入队。
- [ ] fresh 缓存跳过，stale/miss 入队，失败静默降级并由 TTL 重试。
- [ ] 账号/数据库切换取消旧任务、清空队列并阻止旧 generation 写入新库。
- [ ] 私有仓库在入队和执行边界均被拒绝。
- [ ] 补齐并发、去重、启动扫描、新入库、失败与账号切换测试。

## 4. Metadata Wiki 内容与精确重建

- [ ] 扩展 `RAGMetadataSnapshot`，携带三类有效 Wiki 链接。
- [ ] `RAGChunkBuilder.buildMetadata` 按固定顺序输出 Wiki 行并省略空值。
- [ ] `KnowledgeRAGIndexBuilder` 只读 `DiskWikiCache`，不等待网络。
- [ ] Wiki 缓存保存/更新后只刷新对应 repo Metadata。
- [ ] 清空 Wiki 缓存后刷新受影响 Metadata 并移除链接。
- [ ] 无内容变化时不产生无意义 chunk/embedding 工作。
- [ ] 补齐 Metadata 输出、缓存读取、精确重建和清缓存测试。

## 5. Prompt 必带完整 Metadata

- [ ] Repository 批量读取最终仓库有效 `metadata:0`，包含 `keyword_only`。
- [ ] Retriever 将完整 Metadata 作为 repo bundle 固定上下文头，不新增 hit/score/citation。
- [ ] 完整 Metadata 可用时不输出精简元数据。
- [ ] Metadata 缺失或被排除时使用精简元数据兜底。
- [ ] Metadata 自身命中时不重复输出。
- [ ] `structured_only` 保持现有精简元数据语义。
- [ ] evidence 预算优先保留最终仓库完整 Metadata，再裁剪普通分片。
- [ ] 空间不足时减少最终仓库，不保留“无 Metadata 仓库”。
- [ ] 维持没有独立 `{metadataSection}`，不改变 RepoContext 独立占位符。
- [ ] 补齐完整优先、fallback、无重复、structured-only、预算和 citation 测试。

## 6. Metadata 禁删与 Wiki 链接 UI

- [ ] Metadata 行不显示删除按钮。
- [ ] ViewModel / service 拒绝 Metadata 软删除、排除与永久删除。
- [ ] Repository / domain 再次拒绝 Metadata 删除和排除。
- [ ] 其它分片删除、恢复与永久删除行为保持不变。
- [ ] Metadata 仍可查看和编辑 override。
- [ ] Metadata 行展示 DeepWiki、ZRead、CodeWiki 紧凑链接。
- [ ] 链接解析只接受 `http` / `https`，点击使用系统浏览器。
- [ ] 回答 Markdown 外链继续使用现有打开路径。
- [ ] 补齐禁删防线与 Wiki 链接解析测试。

## 7. i18n、正式文档与自动化

- [ ] 新增固定文案进入 `Localizable.xcstrings`，en / zh-Hans 完整。
- [ ] 更新 `docs/3-设计/详细设计/30-本地RAG设计.md`。
- [ ] 更新 `docs/1-立项/开发前问题清单.md`，记录 Wiki Metadata 决策。
- [ ] 更新 `知识库RAG专项进度.md`，不伪造人工验收。
- [ ] 不修改 `docs/功能实现总览.md`，仅起草待确认同步内容。
