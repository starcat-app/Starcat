...
| ready chunk | repo | 维度 | P50 | P95 | 峰值物理内存增量 | 取消延迟 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 18,465 | 1,865 | 1,024 | 4,308.20 ms | 4,372.39 ms | 24.31 MB | 0.20 ms |

结论：400 行分页、Top-K 有界保留和逐页取消使内存与取消行为符合预期；瓶颈是每次查询约 1,890 万维度的纯 Swift 余弦运算，普通 SQLite B-tree 索引不能解决 BLOB 内部相似度扫描。下一步先用系统 Accelerate/vDSP 优化共享余弦内核并在同一快照复测；若大库 P95 仍不能接受，则保留本地边界并在产品中明确引导使用 Qdrant，不追加无证据的 schema migration。
