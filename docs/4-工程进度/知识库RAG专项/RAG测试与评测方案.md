...
### 4.5 长会话启动读取与预取预算基线（2026-07-16）

在 Apple Silicon 开发机的测试宿主内，用 `InMemoryDatabaseManager` 分别构造 100/200 条消息的完整会话。每档执行 5 次热读取，真实经过会话、消息、citation、remote audit 四条固定 SQL 路径，并采样测试进程物理内存；该数据用于同机回归，不是跨设备 SLA。

| 消息数 | SQL 路径 | P50 | P95 | 峰值物理内存增量 |
| ---: | ---: | ---: | ---: | ---: |
| 100 | 4 | 1.404 ms | 1.504 ms | 0.031 MB |
| 200 | 4 | 2.526 ms | 2.708 ms | 0.062 MB |

结论：单会话批量关联读取保持低毫秒级，主要风险来自启动时把最近 24 个完整快照无条件常驻。展示缓存因此保留 24 项 LRU 上限，同时新增 600 条消息与 4 MiB 估算文本双预算；估算覆盖消息、摘要、大纲、引用和可见审计文本，固定大小字段由消息数预算兜底。后台预取不驱逐用户已访问快照，首个无法装入预算的会话会终止本轮预取；超预算会话仍可由用户主动完整加载，只是不进入缓存。
