...
2. **决定是否落到 `docs/3-设计/详细设计/` 或 `docs/工程进度/` 下作为正式设计文档**（按 CLAUDE.md 工作流，这个尺寸的重构需要一个专属设计文档作为单一信任源）。
  ...
  "questions": [
    {
      ...
      "options": [
        {
          "id": "design_doc",
          "label": "A. 先把上面这份架构质量进一步提升，落为 docs/3-设计/详细设计/07-三场景共用架构.md，作为后续实现的单一信任源"
        },
        {
          "id": "more_details",
  ...
  "questions": [
    ...
    },
    {
      ...
      "options": [
        {
          "id": "save_to_docs",
          "label": "A. 把这份完整架构落为 docs/3-设计/详细设计/07-三场景共用架构.md，作为实现的单一信任源"
        },
        {
          "id": "still_grilling",
...
1. **把这份 UI 规范 + 之前的「数据/服务/后端架构方案」合并为一份完整的设计文档** `docs/3-设计/详细设计/07-三场景共用架构.md`，作为后续实现的单一信任源
...
你回答 N1-N5 后，我就把整套架构 + UI + 数据流的最终版本固定下来，可以进入「落到 docs/3-设计/详细设计/07-三场景共用架构.md」环节了。
...
1. 写 `docs/3-设计/详细设计/07-三场景共用架构.md`（单一信任源，含本轮所有决策 + 数据流图 + 改造清单 + 字段映射）
2. 在 `docs/功能实现总览.md` 追加这个重构项的 `- [ ]` 条目（不勾选，标记为"已完成设计，待实现"状态）

要我现在动笔写文档吗？

 dong4j 确认了所有问题的答复：spinner 替代加载图标、button 抖动加短暂红色反馈、AI 窗口的静态决策逻辑、star 成功后覆盖 Hero header 字段。

现在所有 UI、数据和状态机的决策都已敲定，下一步是把这套完整的架构、UI 设计和数据流固定到设计文档里。我应该先简短确认 N1-N5 的所有决策，然后问 dong4j 是否要落到 docs/3-设计/详细设计/ 目录。 现在我在整理最终设计的完整快照，包括状态机层、UI 层的所有决策——从 Star 触发机制、API 反馈策略、按钮视觉状态，到卡片密度、已 star 标记样式、各类徽章颜色和位置的具体规范。
...
```json
{
  "command": "ls docs/3-设计/详细设计/ docs/工程进度/",
...
```json
{
  "path": "/Users/dong4j/Developer/1.AI/ai-incubator/Starcat/docs/3-设计/详细设计/07-Trending数据本地缓存设计.md"
}
```
...
```json
{
  "path": "/Users/dong4j/Developer/1.AI/ai-incubator/Starcat/docs/3-设计/详细设计/17-项目健康度与维护活跃度设计.md",
  "limit": 80
}
```
...
```json
{
  "path": "/Users/dong4j/Developer/1.AI/ai-incubator/Starcat/docs/功能实现总览.md",
  "limit": 120
}
```
...
```json
{
  "path": "/Users/dong4j/Developer/1.AI/ai-incubator/Starcat/docs/4-工程进度/任务录入规范.md",
  "limit": 100
}
```
...
```markdown
OK，结构和编号都清楚了。这次重构应该放在 §6 工程债下作为 **R-01**（独立大型重构），详细方案落到 `docs/3-设计/详细设计/18-三场景共用架构.md`（编号 18，紧接现有 17）。开始写文档。
...
`docs/3-设计/详细设计/18-三场景共用架构.md`（~860 行），结构：
...
`docs/3-设计/详细设计/07-Trending数据本地缓存设计.md` 中「方案 A：复用 `repos` 表」的全部结论已被 R-01 取代。旧文档保留作设计演进史，**不再作为实现依据**——附录 B 已逐章节标注废弃状态。
...
我需要改 `docs/3-设计/详细设计/18-三场景共用架构.md` 以下章节，并把 v1.1 版本变更标到顶部：
...
同时同步改 `docs/功能实现总览.md` §6.6 R-01 条目里**核心产出物**那段（`UnifiedRepoDetailView` 改为 `RepoDetailScaffold + ContentView 插槽`）和**改造规模**（净增 ~1500 行，从原来的 ~1200 上调，因为各 ContentView 是分散写法不是单文件压缩）。
...
/// 触发阈值 + SOP 见 docs/3-设计/详细设计/18-三场景共用架构.md §9.2(详见 `docs/6-发版与上架/v1-上架信息准备.md`)
```

文档 §9.2(详见 `docs/6-发版与上架/v1-上架信息准备.md`) 同步加阈值条目。
