...
> 结论: 发现 3 项功能契约缺口和 4 项文档漂移；均已修复，本轮通过。
...
- P1-01 已关闭：折叠标题行新增七个固定状态点，逐步表达 completed / skipped / failed / cancelled，并保留步骤与状态辅助功能标签。
- P1-02 已关闭：空闲态、生成面板与生成按钮帮助文本均明确 README 和当前个人笔记会发送到用户配置的 AI Provider。
- P2-01 已关闭：等待确认阶段新增“重新生成”，重新冻结当前编辑 buffer 后发起新流程。
- P2-02 至 P2-04 已关闭：详细设计已同步真实文件路径、Prompt 名称、静态流式 helper、应用 API、可观察字段及 i18n 列表。
- 补充生命周期保护：详情 View 销毁时主动取消 AI 流式任务与索引防抖任务。

本轮修复完成后需由专项构建与测试复验；自动化结果统一记录到后续测试审查报告与最终结果报告。
