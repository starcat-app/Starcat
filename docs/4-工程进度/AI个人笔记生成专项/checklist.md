...
> 状态: 已完成（交互增强自动化、四轮复审、折叠回归、交互收口、右对齐、Hero 滚动与最小窗口三栏裁切回归审查通过，人工 UI 验收待执行）
> 创建: 2026-07-22
> 基线: `dev@45aa9763`
> 实施分支: `dev`
...
- [x] 笔记输入框可在详情页原位扩展到 480pt；编辑 / 预览切换右对齐，并默认进入预览。
...
- [x] 交互收口：模式切换右对齐并默认预览、扩展高度提升至 480pt、草稿折叠包含操作区且应用后完全隐藏。
- [x] 布局回归：模式栏显式占满可用宽度并按 trailing 对齐，右侧仅保留 10pt 设计内边距。
