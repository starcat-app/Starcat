...
- [ ] ViewModel 使用可注入单调时钟记录每个已执行步骤的启动时间和冻结耗时。
- [ ] 运行中耗时每 0.1 秒刷新，完成 / 失败 / 取消后冻结，pending / skipped 显示 `—`。
- [ ] 耗时在折叠笔记和切换 repo 后保留，重新生成 / 放弃时重置。
- [ ] AI 草稿标题行右侧新增复制按钮，折叠时仍可复制完整 Markdown。
- [ ] 复制按钮使用 `CopyFeedbackButton`，成功后显示 1.5 秒绿色圆形勾，支持 Reduce Motion、tooltip 和辅助功能。
- [ ] 新增可控时钟单测，并通过专项回归、Direct Debug build、i18n 与静态规范检查。
