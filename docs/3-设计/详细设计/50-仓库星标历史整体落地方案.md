...
> 2026-08-22 架构修订：本文件继续作为当前 v1.3 已实现事实和验收边界；`starcat-discovery-api` 中的 Star History 路径是当前客户端正在使用的过渡实现。长期将数据接收、GH Archive 构建和公共查询迁入独立 `starcat-history-api`，接入用户主动贡献的公开 repo 日快照；迁移稳定后删除 Discovery 路径和第三方 History Provider，不保留永久双轨。
...
1. History 服务实现与当前 `StarHistoryAPI` 兼容的查询契约，以及贡献 ingest、删除重算和模型版本。
2. 一次性导出 Discovery 公开历史缓存，按 repo ID、点数、source/precision 和 hash 校验后导入。
3. Shadow 对比后按 1%/10%/50%/100% 灰度切换查询。
