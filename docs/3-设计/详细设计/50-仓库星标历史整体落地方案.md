...
> 日期: 2026-07-27
> 状态: 已完成；客户端与 Discovery 服务已实现并完成服务部署；M0 已拆为后续专项，生产 GH Archive / BigQuery Provider 保持关闭；剩余人工矩阵已转发布 QA
> 版本: v1.3（关单边界回填）
...
> 后续目标: [Starcat 自研星标历史服务详细设计](62-Starcat自研星标历史服务详细设计.md)

> 2026-08-30 架构修订：本文件只保留 v1.3 的历史事实和验收边界。普通公开仓库已切换到独立 `starcat-history-api` 的原始日事件接口，由 Starcat 使用本机公开 `stars_count` 校准；客户端 History 贡献已取消。Discovery 路径仅保留迁移期回退，生产稳定后删除，不保留永久双轨。当前契约以 62 号设计为准。

## 0. 实现状态

截至 2026-07-27，星标历史的本地代码闭环已经完成：

| 层级 | 已实现 | 当前门槛 |
|---|---|---|
| Discovery | 独立缓存表、BigQuery REST Provider、归一化、降采样、有界 worker、公开性复核、ETag 与稳定错误契约；生产服务已部署并常驻 | M0 已拆为后续专项，未执行真实 BigQuery 查询，生产 Provider 保持关闭 |
| Starcat 数据层 | v16 历史点表、同步链路每日快照、`StarHistoryAPI`、cache-first 合并、私有仓库拦截 | 无 |
| Starcat 状态层 | `StarHistoryViewModel` 的独立范围、generation、取消、三次有界轮询和增长派生 | 无 |
| Starcat UI | 仓库洞察内 `StarHistorySection`、Summary、来源标识、Chart、状态与 VoiceOver 读数 | 完整人工 UI 矩阵转发布 QA |

M0 已于 2026-07-30 从本需求关单范围拆为后续专项，当前结论仍是“**代码实现可验证，数据可行性尚不能 GO**”：Fake / `httptest` 已证明参数化查询、dry run、`maximumBytesBilled` 和预算拒绝逻辑正确，但没有凭据、预算授权和真实扫描结果，不能宣称 GH Archive 质量与成本已通过。未取得 M0 GO 前，生产语义必须保留降级路径：只展示 Starcat 本机精确快照和有权限仓库的 GitHub Stargazers 数据，不启用远端估算曲线。
...
其中“不为该功能新建独立后端项目”只适用于 2026-07-27 的首版实施范围。2026-08-22 已确认长期拆出 `starcat-history-api`，原因是 History 已形成独立的数据接收、聚合、异步构建、查询、删除重算和成本治理职责，不应继续依附 Discovery catalog。
...
- 主详情和独立详情窗口都通过 `ManageDetailContent` 复用 `RepoDetailScaffold`。
- `RepoMetadataHeaderView` 已有 Hero、统计行和 trailing actions。
- Stars 统计 chip 当前负责 Star / Unstar，不能再复用为历史入口，否则一个控件会承担两种冲突行为。
- `ManageDetailContent` 已提供 `README / 洞察`模式，并在洞察模式卸载不可见 README `WKWebView`。
- `AIUsageDashboardView` 已使用 Swift Charts，项目不需要新增图表依赖。
- `AppEndpoints` 是自建服务 endpoint 的统一入口。
- 已追加 `v16-repository-insights`，没有修改 `v1-initial` 或其他已发布 migration。
- `GRDBRepoStarHistoryRepository`、`StarHistoryAPI` 与 `StarHistoryViewModel` 已从 `AppDependencies` 注入。
...
生产 ingest 链路继续在 repo metadata 与 release 成功落库后调用 `RecordDailySnapshot`，并已有防回退测试。任意公开仓库的长期历史缓存、查询 endpoint 和异步构建代码已实现；真正剩余的是 M0 真实数据验证与部署授权。
...
日期下界必须来自服务端 GitHub Repository 响应中的 `created_at`，不能由客户端传入，也不能让所有仓库统一从 GH Archive 覆盖起点扫描：

```text
startDate = max(repository.created_at UTC 日期, GH Archive 覆盖起点)
```

这样既保持创建日期的可信边界，也避免扫描仓库尚未存在时的日表。仓库 Stars 数量不会降低 BigQuery 扫描字节，M0 必须按每个样本裁剪后的日期范围分别执行 dry run。
...
1. 远程估算点按日期排序。
2. 同一 UTC 日期优先保留本地精确点。
3. 以第一条本地精确快照为锚，按比例重新校准锚点之前的远端形状；锚点开始不再混入远端估算点。
4. 精确快照日期之间不伪造中间值；Chart 只连接真实观察点。
5. 当前 repo 的 `starsCount` 比缓存新时，追加今天的精确点。
6. 本地精确段允许下降，因为真实总数可能发生 Unstar。
...
- Starcat `/events` 客户端始终缓存完整日级序列：`3m` 按日、`1y` 仅压缩远端周点并保留全部精确快照、`all` 保留全部真实点。
- 第三方兼容接口可继续限制返回点数；该限制不得反向截断 Starcat 的 `/events` canonical cache。
...
- `all` 的 X 轴从仓库 `created_at` 开始，并在纯渲染序列注入 `(created_at, 0)`；该基线不得写入缓存、数据库或 History API。
- 原始 WatchEvent 日级数据保持完整；`all` 交给 Swift Charts 前使用 LTTB 抽稀到最多 120 点，并强制保留首尾与精度交接两侧。
- `PointMark` 只显示创建点、精度交接点、最新点和当前 hover 点；折线下方使用低透明度面积渐变，不再绘制日级点阵。
- 折线统一线性连接，禁止使用可能越过真实观测值的 Catmull-Rom 平滑。
- 横轴使用包含起止边界的固定数量等距刻度，并保留两端 plot padding，避免重复月份和末项裁切。
...
- `3 个月`：适合观察近期增长。
- `1 年`：默认值。
- `全部`：横轴展示仓库创建日至今天的完整时间域；曲线仍只覆盖实际存在 WatchEvent 或本机快照的日期，不承诺仓库创建以来的精确历史。
...
3. 验证 GH Archive 表结构、覆盖起点、按仓库 `created_at` 裁剪后的查询耗时和扫描字节。
...
### Phase 1：Discovery API（已完成并部署，Provider 默认关闭）
...
部署状态：生产服务已上线并保持 `STAR_HISTORY_ENABLED=false` 默认值；M0 后续专项给出 GO 结论前不得启用 GH Archive / BigQuery Provider。
...
- 舍入后保持单调。
- 一年范围的远端周降采样不丢失末点，且不得丢弃同周内的本机精确快照。
- 全部范围的 Repository 读模型保留同月内每一个事件日，不执行月降采样；图表渲染层使用最多 120 点的 LTTB 抽稀，并保留精度交接边界。
- 全部范围的 X 轴从仓库创建时间开始，渲染层补 `(created_at, 0)` 基线，但不得回写任何持久化数据。
- 同 repo 并发请求只构建一次。
...
| BigQuery 扫描成本高 | 后端成本不可控 | 仓库创建日期裁剪、Phase 0、预算上限、24h 缓存、异步队列 |
...
未来阶段涉及三个独立仓库：Starcat 主项目、现有 `supports/starcat-discovery-api` 和新增 `supports/starcat-history-api`。迁移按以下顺序进行：

1. History 服务发布不可变 Snapshot/Delta，并为 Starcat 提供原始日事件接口。
2. Starcat 使用本机公开 `stars_count` 执行与服务端一致的单锚点校准，继续复用现有 Repository/UI。
3. 对固定样本执行 Discovery shadow 对比后逐步切换生产查询。
4. 停止 Discovery 新建历史任务，观察一个发布窗口后删除路由、Provider、表和环境变量。
5. 删除 SimRepo 或其他第三方 History Provider，禁止长期双写/双读。

详细数据表、群体快照聚合、估算公式、API 和 SLO 以 [62-Starcat自研星标历史服务详细设计](62-Starcat自研星标历史服务详细设计.md) 为准。
...
6. 当前用户拥有、组织成员或外部协作的仓库通过项目关系选择 OAuth / GitHub App
   直连 `/stargazers`；普通第三方收藏仓库继续使用 Discovery / GH Archive 估算。
7. 与洞察中心共用 `v16-repository-insights`，但保持独立数据表和独立 ViewModel。
8. 不在本轮改写 `docs/功能实现总览.md`；待实现并验收后再单独确认。
9. Discovery Star History 是首版已使用的过渡路径；长期迁入独立 History 服务并在稳定后删除旧路径。
10. 用户贡献只包含主动授权的公开 repo 日级 Star 数观测，真实 GitHub 身份和 Private / Internal 数据不上传。
