...
> 日期: 2026-07-27
> 状态: 客户端、Discovery 服务与自动化已实现；M0 真实 BigQuery 验证、部署和最终人工验收待授权
> 版本: v1.2（实现回填）
...
| 层级 | 已实现 | 当前门槛 |
|---|---|---|
| Discovery | 独立缓存表、BigQuery REST Provider、归一化、降采样、有界 worker、公开性复核、ETag 与稳定错误契约 | 未执行真实 BigQuery 查询，未部署 |
...
M0 当前结论是“**代码实现可验证，数据可行性尚不能 GO**”：Fake / `httptest` 已证明参数化查询、dry run、`maximumBytesBilled` 和预算拒绝逻辑正确，但没有凭据、预算授权和真实扫描结果，不能宣称 GH Archive 质量与成本已通过。未取得 M0 GO 前，生产语义必须保留降级路径：只展示 Starcat 本机精确快照，不启用远端估算曲线。
