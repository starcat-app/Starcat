...
Starcat 可以在仓库详情页增加「星标历史」能力，但不能继续照搬 Star History 原有的 GitHub `/stargazers` 实现。
...
- 在 Manage 仓库详情页和独立详情窗口提供统一的星标历史入口。
...
但当前生产代码没有调用 `RecordDailySnapshot`，只有测试覆盖；同时也没有历史查询 endpoint。
...
```mermaid
flowchart LR
    A["仓库详情页"] --> B["StarHistorySheet"]
    B --> C["StarHistoryViewModel"]
    C --> D["RepoStarHistoryRepository"]
    D --> E["本地 v16 历史点"]
    D --> F["StarHistoryAPI"]
    F --> G["starcat-discovery-api"]
    G --> H["历史结果缓存"]
    G --> I["GH Archive / BigQuery"]
    G --> J["GitHub Repo Metadata"]
    K["Repo 元数据同步"] --> D
    K --> E
```
...
- **local-first**：Sheet 打开先读本地，不阻塞首屏。
...
- 每次成功 upsert repo metadata 后调用 `RecordDailySnapshot`。
...
- 只能追加 `registerV16`，禁止修改 `v1-initial`。
...
- `StarHistoryViewModel`：Sheet 状态、范围选择、取消旧任务、派生增长指标。
- `StarHistorySheet`：稳定的 macOS Sheet 布局和状态切换。
- `StarHistoryChart`：只接收已经排序和筛选好的点。
- `RepoDetailScaffold`：只负责入口与 Sheet 展示，不承载数据计算。
...
- 放在 `RepoDetailScaffold.trailingActionsView`。
- 位于 Wiki / 相似推荐 / 知识库按钮之前。
- 仅当 `repo.id > 0` 且 repo 已经持久化时显示。

样式：

- SF Symbol：`chart.xyaxis.line`。
- 28 × 28 pt icon-only Button，与现有 Hero action 对齐。
- `.buttonStyle(.plain)` 后必须 `.focusEffectDisabled()`。
- 使用 `.primary` / `.secondary` 和系统 accent，不使用 `.tertiary`。
- tooltip 与 accessibility label 使用 `repo.starHistory.open`。
- 不给按钮使用“Star 黄色”，避免与 Star / Unstar 状态混淆。
...
```text
┌─────────────────────────────────────────────────────────┐
│ 星标历史   owner/repo            [刷新] [关闭]          │
├─────────────────────────────────────────────────────────┤
│ 42,810 Stars   +1,284 / 30 天   +8,932 / 1 年          │
│ [估算 · GH Archive] [本机精确快照]      [3月|1年|全部] │
├─────────────────────────────────────────────────────────┤
│                                                         │
│                   Swift Charts 折线                     │
│                                                         │
...
- Header 右上关闭统一使用 `SheetCloseButton`。
- 刷新统一使用 `SyncIconButton`。
- Sheet 保持一个稳定内容树，loading / stale / error 不切换整个根布局。
...
| 状态 | 表现 |
|---|---|
| 首次加载且无缓存 | 保留 Sheet 骨架，Chart 区使用紧凑 ProgressView |
...
- repo 切换时取消旧请求，旧 repo 的响应不能覆盖新 repo Sheet。
...
1. 在共享详情 Scaffold 增加入口。
2. 实现 Sheet、summary、source badge 和 Chart。
3. 实现范围选择、hover、刷新、空态和错误态。
4. 补齐全部 i18n key。
5. 验证 Manage 详情与独立详情窗口。

验收：明暗主题、不同窗口尺寸、Reduce Motion、VoiceOver 基本语义和 18 种语言布局无阻塞问题。
...
- `RecordDailySnapshot` 接入生产 ingest 链路。
...
| 项目 | 必需 | 主要改动 |
|---|---:|---|
| Starcat 主项目 | 是 | v16 数据表、API、Repository、快照写入、ViewModel、Swift Charts Sheet、i18n、测试 |
...
- 远程失败不清空缓存。
- Manage 和独立详情窗口行为一致。
- Sheet 符合 macOS UI、明暗主题、i18n 和辅助功能规范。
...
7. 不在本轮改写 `docs/功能实现总览.md`；待实现并验收后再单独确认。
