...
- `all` 的 X 轴从仓库 `created_at` 开始；创建日至第一条有效数据之间只留空白，不补零、不回填首值。
- 每个 WatchEvent 事件日和本机快照都绘制 `PointMark`；`all` 不叠加面积层，控制十年逐点曲线的绘制成本。
...
- 全部范围保留同月内每一个事件日，不再执行月降采样或固定 400 点抽稀。
- 全部范围的 X 轴从仓库创建时间开始，空白覆盖期不得补造数据点。
- 同 repo 并发请求只构建一次。
