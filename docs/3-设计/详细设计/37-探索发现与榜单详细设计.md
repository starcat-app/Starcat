...
- `popular / newReleases` 从 `starcat-discovery-api/languages` 读取。
...
- `feed` 结果每 `(platform, topic, day)` 缓存到内存 3 小时。
- 榜单接口只读排名表,不实时计算。
- SQLite 开 WAL。
...
- `DiscoveryContentViewModel` 使用 10 分钟内存缓存。
- 切换 mode / filter 时取消旧请求。
