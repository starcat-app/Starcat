...
| `language` | `all` | 语言 |
| `platform` | `all` | 可选平台 |
| `sort` | `score` | `score/stars/activity` |
| `page` | `1` | 1-based |
| `limit` | `20` | 最大 50 |
...
| `language` | `all` | 语言 |
| `platform` | `all` | 可选平台 |
| `sort` | `released` | `released/updated/stars` |
| `page` | `1` | 1-based |
| `limit` | `20` | 最大 50 |
...
- 本地 SQLite 新增 4 张 discovery 缓存表:
...
- `DiscoveryRepository` 提供 `cachedPage`、`fetchPage`、`cachedSummary`、`fetchSummary`:
  - `cached*`: 只读 SQLite,用于快速首屏。
  - `fetch*`: 拉远端成功后替换缓存;远端失败且缓存存在时返回缓存;无缓存才抛错。
- `ExploreDiscoveryViewModel.reload`:
  - 新筛选先读 page=1 缓存并上屏。
  - 再请求远端覆盖。
  - 若新筛选无缓存且远端失败,清空旧列表并显示错误,避免展示不匹配结果。
- `ExploreCatalogStore`:
  - 管理 summary 会话态。
