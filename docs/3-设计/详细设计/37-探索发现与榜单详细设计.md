...
| `CACHE_TTL_SECONDS` | 否 | `900` | 接口缓存建议值 |
...
| `GET` | `/api/v1/discovery/categories/trending` | 是 | 新趋势候选,首期仅后台对比 |
...
`GET /api/v1/discovery/summary` 专门服务 Starcat 左侧 Sidebar。它一次性返回 4 个探索子模式的 repo 总量和可展示筛选项数量,避免客户端为了数字触发多次分页请求。
...
- `popular/new_releases/trending` 的语言计数优先基于对应 `category_rankings(category, bucket='__all__')` 聚合;ranking 尚未生成时回退 `repos` 语言聚合。
...
- `repos`: 全量公开 discovery repo 快照,包含基础字段、主题、平台、release 字段和 `trending_score / popularity_score / release_score / discovery_score`。
...
- `bulk` 读取 SQLite 全量快照,服务端 6 小时内存缓存,支持 gzip 与 ETag,同步成功后主动失效。
- 榜单接口只读排名表,不实时计算。
- SQLite 开 WAL。
...
- 本地 SQLite discovery 缓存分两类:
  ...
  - `discovery_summary_modes`: 4 个模式总量。
...
- `ExploreDiscoveryViewModel.reload`:
  - 非手动刷新先读本地 bulk 并上屏。
  - bulk 在 12 小时 TTL 内不主动请求远端;过期或手动刷新才请求 `/bulk`。
