...
| `CACHE_TTL_SECONDS` | 否 | `900` | `/bulk` 进程内缓存 TTL |
...
- 服务端内存缓存由 `CACHE_TTL_SECONDS` 控制,默认 900 秒,同步成功后主动失效。
...
- `bulk` 读取 SQLite 全量快照,服务端内存缓存 TTL 由 `CACHE_TTL_SECONDS` 控制,支持 gzip 与 ETag,同步成功后主动失效。
- 榜单接口只读排名表,不实时计算。
- SQLite 开 WAL。
