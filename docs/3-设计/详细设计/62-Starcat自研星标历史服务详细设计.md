...
| `all` | Repository 读模型保留全部事件日和全部精确快照，不按月聚合；Swift Charts 渲染前另做最多 120 点的 LTTB 视觉抽稀 |
...
图表 `all` 的 X 轴从 GitHub 仓库创建时间开始，并在纯渲染序列中注入 `(created_at, 0)` 基线；该点不进入 Repository 缓存、数据库或 History API。原始事件仍完整用于统计与缓存，图表仅使用 LTTB 保留首尾、主要拐点和精度交接两侧，默认最多渲染 120 点。可见 `PointMark` 只保留创建点、精度交接点、最新点和当前 hover 点，折线继续使用线性连接，并叠加低透明度面积渐变。`3m/1y` 的 X 轴不早于仓库创建时间。
