...
> 关联：
> - `docs/3-设计/详细设计/06-核心模块设计.md`（Sync / Repository 模块边界）
> - `docs/3-设计/详细设计/16-活动页设计.md`（Activity 聚合页定位）
> - `docs/3-设计/详细设计/07-Trending数据本地缓存设计.md`（**部分取代**，见附录 B）
...
flowchart TB
    subgraph 后端
        ...
        BW[weekly API<br/>/api/v1/weekly<br/>SQLite + enricher 扩字段]
    end
...
>
> 1. share / ai 行为对所有 repo 完全一致：share 必走 AI 摘要 + 分享卡；ai 的
>    外部入口统一展开详情页底部 `RepoAIFloatingOverlay`。
> 2. `RepoAIWindowController` 保留为底部面板内部“在独立窗口中打开”的附属展示
>    形态，不与底部面板并列成为外部主入口。Scaffold 统一挂载底部面板并发布 AI
>    可用性，外部 handler 没有注入点。
...
#### 3.2.11 hero 入场动画（4 详情页同构 — D-28 v3 重构, 2026-06-11）

> ⚠️ **强制契约**：4 个详情页（manage / trending / weekly / activity-repo-backed）**必须**走同一种「Shell + 外层 .id 重建 + `.detailContentTransition()`」模式。本节是 R-01 §3.2 的子契约，新加详情页场景必须遵循。

##### 背景

R-01 v1.2 Phase B5 把渲染层迁到 `RepoDetailScaffold`，但**外壳没拆**——`WeeklyDetailView` / `ActivityDetailView` 自持 `@State displayRepo` / `@State readmeVM`，导致同分支切换 item 时无法走「shell .id 重建」路径，hero 入场动画无法稳定触发（dong4j 2026-06-11 反馈"看不到动画"，D-28 v1/v2 修复未果）。

##### Shell 模式契约

每个 repo-backed 详情场景**必须**有独立的 `*ScaffoldShell` view：

| 场景 | Shell 文件 | 输入 prop |
|---|---|---|
| manage | （无独立 shell，由 `RepoDetailView` 内部装配 `RepoDetailScaffold`，prop 同步流入） | `Repo`（来自 `viewModel.selectedRepo`） |
| trending | `Features/Trending/TrendingScaffoldShell.swift` | `TrendingRepo` |
| weekly | `Features/Activity/WeeklyDetailScaffoldShell.swift` | `WeeklyProject` |
| activity-repo-backed | `Features/Activity/ActivityDetailScaffoldShell.swift` | `ActivityItem` |

**Shell 必满足三条契约**：

1. **自持解析层 @State**：`displayRepo` / `readmeVM` / `isLocalHit` 等全部下沉到 shell 内部 `@State`。外层路由 view（`WeeklyDetailView` / `ActivityDetailView`）**不持有任何解析层 state**，只做"路由 + 空态 + non-repo 自绘"。
2. **同步快路径**：`.task(id: prop.id)` 入口必须**同步**推 displayRepo（不 await 任何网络），保证第二帧立即有内容。
   - trending：`trending.makeEphemeralRepo()` 同步构造（不调网络）
   - weekly：`makeFallbackRepo(from: project)` 同步构造（D-27 修法），后台 `resolveRepo` silent upgrade 调 GitHub `/repos`
   - activity-repo-backed：直接 `displayRepo = item.repo`（item.repo 已是本地真值）
3. **重建友好**：外层挂 `.id(prop.id)` 时整个 shell 重建，@State 自动重置 → 无 stale 渲染（与 D-27 ghosting 修法兼容）。

##### 外层路由层契约

外层路由 view（`RepoDetailView` / `WeeklyDetailView` / `ActivityDetailView`）**必须**：

```swift
ZStack(alignment: .topLeading) {
    if let prop {
        XxxScaffoldShell(prop: prop)
            .id(prop.id)               // ← 关键 1：让 shell 重建
            .detailContentTransition() // ← 关键 2：非对称 transition
    } else {
        emptyState
            .id("xxx-empty")
...
.animation(.easeOut(duration: 0.4), value: prop?.id ?? "xxx-empty")
```

**4 处约束（缺一不可）**：

1. **`ZStack(alignment: .topLeading)` 必须**：Group transparent container 跨分支切换 transition 不稳定触发（详见 `Shared/Components/DetailContentTransition.swift` 文件头）。
2. **shell 外层挂 `.id(prop.id)`**：让 shell 重建路径生效，@State 重置 + transition 触发。
3. **`.detailContentTransition()` 共享 modifier**：非对称 transition（insertion: `opacity + offset y:14` / removal: 仅 `opacity`），accessibility reduceMotion 时回退纯 opacity。
4. **`.animation(.easeOut(duration: 0.4), value: prop?.id ?? ...)`**：与外层 `.id` 同源，让 transition 在 0.4s 内完成插值。

##### 视觉效果

| 场景 | 触发 | 动画 |
|---|---|---|
| manage 切 repo | `RepoDetailView` 内部 `viewData` prop 同步流入 + `RepoDetailScaffold` 内部 `VStack.id(repo.id)` 重建 + 父级 `.animation(value: detailContentID)` 包裹 | opacity 0→1 渐变 0.4s（隐式实现，prop 同帧到达） |
| trending 切 repo | shell 重建 + `.detailContentTransition()` insertion | "轻轻落下"：opacity 0→1 + offset y:14→0，0.4s |
| weekly 切 project | shell 重建 + `.detailContentTransition()` insertion | "轻轻落下"：同 trending |
| activity 切 item | shell 重建 + `.detailContentTransition()` insertion | "轻轻落下"：同 trending |

> 注：manage 不挂外层 `.id` 是因为 `viewModel.selectedRepo` 是同步 prop，不经 @State 异步推送 → `RepoDetailScaffold` 接到新 repo 时 `VStack.id(repo.id)` 与父级 `.animation(value: detailContentID)` **同帧变化** → SwiftUI 用 0.4s easeOut 包裹。视觉上 manage 是 opacity 渐变，trending/weekly/activity 是 opacity + offset 滑入；二者细节不同但都属"切换有视觉反馈"，dong4j 验收时无需追求像素级完全一致。

##### 不挂外层 .id 的反例（D-28 v1/v2 失败教训）

❌ **错误做法**：把 `.id` 挂在分支稳定标识上（如 `"weekly-content"`），让 content 子树不重建以避免 ghosting。
- 问题：shell 不重建 → @State displayRepo 跨 prop 持久化，但 displayRepo 通过 `.task` 异步推送 → 与 `weeklyBranchID` 不同帧变化 → `.animation(value:)` 单帧包裹语义抓不到 → RepoDetailScaffold 内部 VStack 重建无包裹 → **瞬切无动画**。

✅ **正确做法**（D-28 v3）：把 @State 搬进 shell，外层挂 `.id(prop.id)` 让 shell 重建。@State 跟 shell 一起重置，避免 ghosting；`.task` 同步快路径保证第二帧有内容；transition + animation(value:) 完整触发。

##### 新加场景的 checklist

新加 repo-backed 详情页场景时（如未来的 sharing / playground / radar 等），必须：

1. 抽 `XxxScaffoldShell.swift`，输入仅 `let prop: XxxXxx`（非 optional）
2. 在 shell 内部持有所有解析层 @State（displayRepo / readmeVM 等）
3. 在 shell 内部 `.task(id: prop.id)` 同步推 displayRepo + 触发 readmeVM 加载
4. 外层路由 view 走标准模板：`ZStack + .id(prop.id) + .detailContentTransition() + .animation(value:)`
5. README VM 局部持有，不复用 HomeView 全局 readmeVM（避免污染主路径）
...
final class StarredRegistry {
    ...
    // ⚠️ 已知扩展点（达到阈值再做，详见 §9.2(详见 `docs/6-发版与上架/v1-上架信息准备.md`)）：
    ...
    // 触发阈值参考 §9.2(详见 `docs/6-发版与上架/v1-上架信息准备.md`)「已知约束」。
}
...
struct BackendAggregateRepoSource: RepoSource {
    ...
    func tryResolve(owner: String, name: String, hint: StarcatRepoCardDTO?) async throws -> Repo? {
        // 实现：先 hint 命中就跳过 (本来就该在上面 source 命中)；否则 GET /api/v1/weekly?owner=&repo=
...
| Weekly 列表 | starcat-weekly-api `/api/v1/weekly` | ❌ 不做（空态 + retry） |
...
| `GET /api/weekly/projects` | `GET /api/v1/weekly` | weekly 列表 |
| - | `GET /api/v1/weekly/{owner}/{repo}` | weekly 单 repo（给 BackendAggregateRepoSource 用，可选实现） |
...
**接口响应改造**：
- 旧 `GET /api/weekly/projects` → 新 `GET /api/v1/weekly`（v0.5.2 dong4j 重命名）
...
- 单 repo（给 `BackendAggregateRepoSource` 用）：`GET /api/v1/weekly/{owner}/{repo}` 可选实现，R-01 内**可不做**（chain 跳过此源继续询问 GitHub）
...
| `supports/starcat-weekly-api/internal/handler/weekly.go` | 响应改 `StarcatRepoCardResponse`，endpoint 改 `/api/v1/weekly`（v0.5.2 进一步重命名），加 `weekly` 扩展段 |
...
Step 1: 后端 schema 改造
        ...
        └── weekly: enricher 扩 13 字段 + schema migration + /api/v1/weekly
...
| R2.2 | 启动期 SELECT 大规模下会卡吗 | 接受预案：R-01 内不做，但在代码注释 + §9.2(详见 `docs/6-发版与上架/v1-上架信息准备.md`) 留 Snapshot 触发阈值（> 5000 starred） |
...
`docs/3-设计/详细设计/07-Trending数据本地缓存设计.md` 中以下内容**已废弃**：
