...
> 状态：✅ dong4j 已确认按 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).2「推荐执行顺序」实施。
...
> 完成后必须勾选 `[x]` + 在 `docs/功能实现总览.md` 加 `> 实现：...` 行 + 变更日志加一行（详见 CLAUDE.md / AGENTS.md 工作流约定）。
...
- [x] **T1.11** 更新 `docs/功能实现总览.md`：3.3 节追加 `[x] README 缓存软过期 + 404 session 缓存` + 实现说明；顶部「最近更新」+「变更日志」+ 仪表盘（33 → 34，47 → 52）。
...
- [x] **T2.2** `Starcat/Core/Network/GitHubAPI/ReadmeAPI.swift`：新增 enum `ReadmeRefreshResult`：— **2026-05-30 21:00 完成**
  ...
  ```
  > 实现：完全按 §13(详见 `docs/3-设计/详细设计/23-Chrome-插件方案.md`) 设计落地。`notModified(Readme)` 携带的是已更新 `cachedAt` 字段的 Readme（不是原值），便于 ViewModel 直接更新 UI 显示"刚刚刷新"。
...
- [x] **T2.5** `Starcat/Features/Home/ReadmeViewModel.swift`：重写 `load(repo:)` 为两段式（实际是**三阶段**）：— **2026-05-30 21:00 完成**
  ...
  >   - .failed(error) → 见 T2.6
  > ```
  > **`isRefreshing` 状态不暴露 UI**（按 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).1 决策），保持原 5 态枚举。**完成 T2.5 即完成 T2.6**（一并实现）。
...
- [x] **T2.11** 更新 `docs/功能实现总览.md`：— **2026-05-30 21:00 完成**
...
- [x] **TX.2** §10「最终我会采用的版本」校对实际实现 — **2026-05-30 21:30 完成**
  ...
  >
  > **差异 2（命名）**：§10 图中 `assembleDocument(fragment, isDark, cssVersion)` → 实际签名 `assembleDocument(fragment:isDark:)` **无 cssVersion**。原因：已在 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).3 订正点 2 说明 —— SwiftUI `NSViewRepresentable` 在 Coordinator 持有 `lastLoadedKey: ReadmeKey` 做去重，CSS 修改只需开发者改完重启 App 即可生效；运行时 CSS 不会动态变化，`cssVersion` 是多余的 cache buster。
  >
  ...
  >
  > **差异 5（精细化）**：§10 图中 `404: 缓存 not_found` → 实际实现为内存 Set `sessionNotFound`（不持久化），而非数据库列。原因：见 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).1 Phase 1 评审决策 —— 持久化 not_found 状态意味着 README 后续被作者补上时用户不会自动看到，session 缓存兼顾性能与新鲜度。文档与实现一致（§5.3 和 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).1 都明确了这个设计）。
- [~] **TX.3** ~~把 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).3 两点订正同步告知原文档作者（如果是外部评审），避免对方在其他项目里基于同样的误解再做决策。~~ — **N/A（不适用）**
  > 实现说明：此设计文档为 Starcat 项目内部评审产物（dong4j 与 AI 协作者迭代写成），没有"外部作者"需要同步。订正已直接写在 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).3 章节内供本项目所有后续协作者参考。条目标记为 `[~]` 表示 "已主动取消，无需后续处理"。
