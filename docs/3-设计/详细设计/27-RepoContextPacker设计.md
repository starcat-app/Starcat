...
>
> ⚠️ **v1.2 实施权威**：写代码前**必读 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`)「实施前 grill 决策记录」**。§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 是 10 轮 grill 拍板的实施权威覆盖层，与 §1-§21(详见 `docs/3-设计/详细设计/21-weekly-api-后端3源聚合改造.md`) 冲突时**一律以 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 为准**。§1-§21(详见 `docs/3-设计/详细设计/21-weekly-api-后端3源聚合改造.md`) 保留作为「为什么这么设计」的过程背景。

---

## 0. 客户端接入任务清单（2026-06-13 起，**当前在做**）

> **本章是"立即可执行的工程任务列表"**，与 §23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`)「实施进度」配套（§23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 记录已落地的底层 pipeline，§0 记录当前接入阶段的待办）。
>
...
> 触点编号沿用 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`) 设计文档命名（A~F），独立实施可并行。
...
- [ ] **Y7. 端到端 fixture 测试** —— `StarcatTests/RepoContextPacker/EndToEndIntegrationTests.swift`：
  ...
  - 跑完整 `RepoContextPacker.pack(_:)`，断言：
    ...
    - 输出 `metadata.json` 解析成功且 `actualTokens ≤ tokenBudget × 1.2`（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).7 决议）；
...
- 设计文档 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).11(e) 提到的 mojibake 文件名实际累计 warnings 逻辑；
- V2 ⭐⭐⭐ 优先级：tiktoken-swift 精确 token 计数 / `.gitignore` 解析；
- V2 ⭐⭐ 优先级：Markdown 输出格式；
- V2 ⭐ 优先级：tree-sitter compress / git 历史增强。
- 详见 §23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).3 W/X/Y 表后的 Z 段、§23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).6 后续任务粒度估算。
...
4. **进度同步**：`docs/功能实现总览.md` RepoContextPacker 条目「客户端接入（Step 8-10）」`[ ]` → `[x]` + `> 实现：...` 行。
...
> ⚠️ **v1.2 覆盖**：MVP 实施依赖项已确定 —— ZIPFoundation（[§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).2](#222-q1--zip-解压库--zipfoundation)）、自写 glob→regex 转换器（[§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).3](#223-q2--glob-匹配--自写-glob--regex-转换器)）。临时目录与产物布局以 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).6 Q5 决议](#226-q5--临时目录--产物布局) 为准（解压用系统 `temporaryDirectory`、产物持久化到 `Application Support/Starcat/analysis/<owner>/<repo>/`）。所有 5 项安全防护见 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).11 Q10 决议](#2211-q10--5-项安全防护)。
...
> ⚠️ **v1.2 覆盖**：截断语义（Tier 1 行数 80 + 字符数 4000 双约束 + 统一 `// ... [truncated: ...]` marker、Tier 0 100KB 硬上限降级 Tier 2、单文件 5MB 上限降级 Tier 2）由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).9 Q8 决议](#229-q8--截断语义) 定型；二进制检测策略（扩展名白名单 fast-path + NUL 字节 8KB 探测）由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).7 Q6 决议](#227-q6--二进制检测--扩展名白名单--nul-探测) 定型。本节的「Tier 0 / Tier 1 / Tier 2」清单（精确文件名、glob 模式）仍然有效，**仅截断与二进制检测策略以 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 为准**。
...
> ⚠️ **v1.2 覆盖**：Token 估算公式 / 估算时机 / 校准策略 / 超 budget 兜底已由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).8 Q7 决议](#228-q7--token-估算两阶段估算--校准) 重新拍板（两阶段：Pass 2 用 size 估算 → Pass 3 用真 char count 校准 + 不回滚 plan + warning 兜底）。本节描述的"立即估算文件内容"在 v1.2 改为"Pass 2 用 size 估算，Pass 3 真读才校准"。
...
> ⚠️ **v1.2 覆盖**：XML 模板字段、`metadata.json` 字段、stats 内容已由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).10 Q9 决议](#2210-q9--xml-输出格式) 重新定型；新增根元素属性 `schemaVersion` / `tierRulesVersion` / `tokenEstimatorVersion` / `tokenBudget`，stats 新增 `estimatedTokens` / `actualTokens` 双值。CDATA 转义、属性值转义工具见 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).10 `XMLEscape` 实现。**实施时以 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).10 为准**。
...
> ⚠️ **v1.2 覆盖**：模块的 actor / struct 类型由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).5 Q4 决议](#225-q4--三-pass--pass-3-taskgroup-并发读--全-struct) 修订——**`SourceZipExtractor` / `BudgetAllocator` / `ContextWriter` / `RepoContextPacker` 四个原标 actor 一律改为 struct**（无共享 mutable state，actor 反而引入 hop 成本）；`XmlOutputBuilder` 改为 `async throws`（内部 Pass 3 用 TaskGroup cap=8 并发读 Tier 0/1）；具体签名以 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).5 表为准。本节代码示例的 `actor` 关键字、原 §9.1(详见 `docs/6-发版与上架/v1-上架信息准备.md`) 临时目录策略、§9.4(详见 `docs/6-发版与上架/v1-上架信息准备.md`) TokenEstimator 公式、§9.5(详见 `docs/6-发版与上架/v1-上架信息准备.md`) BudgetAllocator 估算时机均已被覆盖。
...
> ⚠️ **v1.2 覆盖**：错误枚举由 [§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).4 Q3 决议](#224-q3--分层错误处理) 重新定型为「分层错误」——致命错抛、单文件错 skip 写入 `metadata.json.skippedFiles[]`。下面这份 v1.0 枚举**已不完整**：v1.2 新增 `zipSlipDetected` / `extractedDirectoryTooLarge`、移除 `fileReadFailed`（改为 SkipReason 字符串常量）、`tokenBudgetExceededByTier0` 已合并到「Pass 3 校准 + warning 兜底」机制（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).8）。**实施时以 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).4 枚举为权威**。
...
| 6 | **i18n 优先** | 所有用户可见文本走 String Catalog，命名空间 `ai.context.*`（详见 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).9） |
...
|---|---|---|---|
| 网络断（ZIP 下载失败） | 阶段 1.2 | banner（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).6） + 「重试」按钮 | 检查网络后重试 |
...
```
ai.context.phase.*          摘要生成阶段（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).3）
ai.context.footer.*         摘要 footer 元信息（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).4）
ai.context.settings.*       AI 设置页配置区（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).5）
ai.context.banner.*         降级 banner（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).6）
ai.context.storage.*        数据管理面板（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).7）
ai.context.menu.*           右上角菜单项（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).8）
ai.context.error.*          错误文案（§12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).11）
```
...
| `docs/3-设计/详细设计/14-AI集成落地记录.md` | AI 调用链由 `RepoAIInsightService` 编排，本方案只负责 context.xml，prompt 编排不动 |
| `docs/3-设计/详细设计/15-AI设置与调用链重构方案.md` | AI provider / model 抽象不动；本方案新增的 AppSettings 字段遵循同款 `AIModelTask`-style 设计 |
| `docs/3-设计/详细设计/26-向量搜索改进.md` | 向量搜索的 `IndexedTextBuilder` 与本方案的 Packer **完全独立**——前者服务于向量化，后者服务于 AI 摘要 prompt 注入；两者输入源都包含 README，但输出格式和消费者都不同 |

---

## 22. 实施前 grill 决策记录（v1.2 实施版本附录）

> **本章是 §1-§21(详见 `docs/3-设计/详细设计/21-weekly-api-后端3源聚合改造.md`) 的【实施权威覆盖层】**——v1.2 通过 10 轮 grill 对 10 个关键技术决策点做了**最终拍板**，所有决议**优先于**原章节的描述。代码实施时遵循本章；§1-§21(详见 `docs/3-设计/详细设计/21-weekly-api-后端3源聚合改造.md`) 保留作为「为什么这么设计」的背景上下文。
>
...
**实施代码**（覆盖原 §9.1(详见 `docs/6-发版与上架/v1-上架信息准备.md`)）：
...
**TokenEstimator 实现**（覆盖原 §9.4(详见 `docs/6-发版与上架/v1-上架信息准备.md`)）：
...
| Tier 1 行数上限 | 80 行 | ✅（AI 设置页 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`).3 D） | |
...
1. **通用 unzipped root 识别** — `findUnzippedRoot(in:)` 见 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).6
...
- [x] **错误枚举**：严格按 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).4 实现（11 个 case，已剔除 `fileReadFailed` 和 `tokenBudgetExceededByTier0`），未照搬原 §10
- [x] **模块签名**：严格按 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).5 实现（4 个原 actor 全降级为 struct），未照搬原 §9
- [x] **XML 输出**：严格按 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).10 模板实现（含 5 段保留 / CDATA 拆段转义 / 3 个版本属性 / 2 空格缩进）
- [x] **TaskGroup cap = 8**：常量定义在 `TierRules.contentReadConcurrencyCap`（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).5）
...
- [x] **测试覆盖**：8 个测试文件 ~55 case 覆盖 grill 10 项决议边界（详见 §23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).3）
...
>
> **本次范围**（dong4j 明确决策）：「先实现从 ZIP 解压、代码分析到生成 XML 的这一流程；下载 ZIP 和之后 XML 如何使用的逻辑先不管」 → 即设计文档 §18(详见 `docs/3-设计/详细设计/18-三场景共用架构.md`) 的 **Step 1-7 全部完成**，**Step 8-10 客户端接入暂不做**。
...
- [x] **B1. `Models/RepoContextPackerError.swift`** —— 11 个致命错 case（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).4）
- [x] **B2. `Models/SkipReason.swift`** —— 6 类单文件错 reason 常量 + `SkippedFile` 结构体（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).4）
...
- [ ] **U2. 端到端 ZIP fixture 集成测试** —— 设计文档 §18(详见 `docs/3-设计/详细设计/18-三场景共用架构.md`) Step 10「集成测试 + UI 接入」范围；MVP 验收预期用 4 个 fixture ZIP（vapor/vapor / repomix 自身 / 大型 monorepo / 小型 demo）跑端到端验证「< 1s 出 context.xml」「token 估算误差 ≤12%」等指标。**未做原因**：dong4j 明确「先实现 pipeline」，集成测试与 §19(详见 `docs/3-设计/详细设计/19-wiki集成.md`) 验收标准一起在客户端接入阶段做。
...
- [ ] **U4. UTF-8 decode 失败统一记 `encodingDetectionFailed`** —— XmlOutputBuilder 读单文件时把「读不开 / 权限不足 / UTF-16 编码 / 真损坏」全部合并为 `encodingDetectionFailed`。**接受原因**：对消费方区分这些来源意义不大；MVP §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).7 决议明确不做编码探测。
- [ ] **U5. mojibake 文件名不处理** —— §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).11 (e) 决议；< 0.001% 触发率；目前未追加 `warnings: ["nonAsciiFilenamesDetected: N"]` 实际累计逻辑（因为没读全 enumerator 文件名做 ASCII 检测），代码层 warnings 数组初始为空。**后续行动**：如果生产环境真碰到投诉，加 N 行检测逻辑即可。
- [ ] **U6. `.gitignore` 解析** —— MVP 不做，默认 ignore 列表 + 文本扩展名白名单已足够（§22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).11 / 原 §13(详见 `docs/3-设计/详细设计/23-Chrome-插件方案.md`)）；V2 ⭐⭐⭐ 优先级。

### 23.3 待办（客户端接入阶段，本次未做）

> 设计文档 §18(详见 `docs/3-设计/详细设计/18-三场景共用架构.md`) Step 8-10 范围，dong4j 决策本次不做，单开后续任务。
...
- [ ] **Z6. ❌ 永不做**：MCP server / Skill 生成 / 远程 git clone / secretlint / CLI 入口（每条都有 §1 / §14(详见 `docs/3-设计/详细设计/14-AI集成落地记录.md`) / §16(详见 `docs/3-设计/详细设计/16-活动页设计.md`) 决议背书）
...
*变更记录：*  
*- v1.4（2026-06-13 21:50）：新增 §0「客户端接入任务清单」放到文档最前面，作为当前阶段的可执行任务索引；按 W → X → Y 三阶段拆分（W = SharedSnapshotService + RepoAIContextProvider 抽象层 / X = AppSettings 4 字段 + RepoAIInsightService prompt 注入 / Y = 6 个 UI 触点 + E2E fixture）；§0.1 现状摸底表对齐 8 个接入面到具体代码行号；§0.6 DoD 给硬性完成标准；§0.7 风险清单列出 5 项已识别风险与缓解。与 §23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 配套：§23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 记录已落地的底层 pipeline，§0 记录当前接入阶段的待办。*  
*- v1.3（2026-06-13 17:30）：新增 §23(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`)「任务清单与实施进度」章节；标注本次（2026-06-13 17:15）已完成 §18(详见 `docs/3-设计/详细设计/18-三场景共用架构.md`) Step 1-7 全部范围（18 新源文件 + 8 测试文件 ~55 case + ZIPFoundation 依赖集成 + AboutDependency 同步），Step 8-10 客户端接入（W1-W3 / X1-X5 / Y1-Y7 共 ~3.5 天）和 V2 升级（Z1-Z5）暂未做；同步勾选 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`).12 实施前检查清单（全 9 项 ✅）；`SkipReason` 实际归位 `Models/` 而非 `Rules/`（语义上与错误处理配对）。*  
*- v1.2（2026-06-13 16:21）：新增 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`)「实施前 grill 决策记录」作为实施权威覆盖层；记录 10 轮 grill 全部决议（ZIPFoundation 依赖 / 自写 glob→regex / 分层错误 / 三 pass + TaskGroup cap=8 / 系统 temp 解压 + Application Support 持久化 / 扩展名白名单 + NUL 探测 / Token 两阶段估算→校准 / Tier 1 行字符双约束 + Tier 0 100KB 上限 / String 拼接 XML + CDATA 拆段转义 / 5 项安全防护）；§9 / §10 / §6 章节由 §22(详见 `docs/3-设计/详细设计/27-RepoContextPacker设计.md`) 覆盖；__SourceZipExtractor / BudgetAllocator / ContextWriter / RepoContextPacker__ 由 actor 改为 struct；错误枚举补 `zipSlipDetected` / `extractedDirectoryTooLarge` / `cancelled`，移除 `fileReadFailed`（改为 SkipReason）。*  
*- v1.1（2026-06-13 14:55）：新增 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`) UX 设计章节（15 个子节）；后续 §12(详见 `docs/6-发版与上架/v1-上架信息准备.md`)-§20(详见 `docs/3-设计/详细设计/20-wiki-api-对接.md`) 顺移到 §13(详见 `docs/3-设计/详细设计/23-Chrome-插件方案.md`)-§21(详见 `docs/3-设计/详细设计/21-weekly-api-后端3源聚合改造.md`)；定义 8 个 UI 触点（5 个 MVP + 3 个 V2）、失败处理矩阵 7 项、i18n 命名规范 7 个子命名空间。*  
