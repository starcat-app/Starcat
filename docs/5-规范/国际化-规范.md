...
7. **禁止用脚本读写该目录**（Python / `json.dump` / `jq` 写回 / `xcstrings_patch.py --apply` / 截断后拼接 HEAD）。只允许用相邻已有 key 做唯一关键词匹配的局部插入或修改。改完看 `git diff --stat`：只应增加几十行；出现上万行删除必须停手，禁止再跑抢救脚本。详见 [`i18n-军规.md`](i18n-军规.md) 铁律 #6。
