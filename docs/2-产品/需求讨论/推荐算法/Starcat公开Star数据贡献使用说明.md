...
Starcat 的“匿名贡献公开 Star 数据”默认关闭。从 1.4.0 起，已登录用户会在启动遮罩和首次
引导结束后看到一次贡献授权提示；提示按 GitHub 账户隔离，每个账户只出现一次。未登录时
不会弹出或消耗提示，已在设置页开启贡献的账户也不会再被打扰。

提示中的开关默认关闭。选择“暂不参与”或关闭窗口都不会开启贡献；选择“保存并继续”后，
仍可随时在“设置 → 通用 → 隐私与推荐”修改。授权提示同时提供 Starcat 开源地址
`https://github.com/starcat-app/Starcat`，用户可以直接审核数据采集与上传实现。

开启代表允许当前 GitHub 账户在下一次**正常成功的 Stars 完整同步**后，静默上传以下字段：
