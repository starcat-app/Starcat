...
>
> 状态：**§8 核心项已拍板（2026-06-18）**；实施规格以 **`docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md`** 为单一信任源。
>
> 前置阅读：`docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md`（拍板结果 + 技术方案）、`docs/1-立项/功能清单.md` §7、`docs/1-立项/需求分析.md` §7。
...
随着接入 StoreKit 2（`docs/功能实现总览.md` §5.3，标记 P2）的时点临近，需要提前明确**哪些能力是免费版的「基础工具价值」，哪些进入 PRO 订阅的「增值能力」**。否则会出现以下风险：
...
`docs/1-立项/需求分析.md` §7 和 `docs/1-立项/功能清单.md` §7 已经给出了初步定价方案：
...
本文档基于 `docs/功能实现总览.md` 的最新清单做精细化重梳理。
...
### 原则 ⑤ 数量限制用于基础功能，AI 走 Pro 硬边界

对于免费 + Pro 都可访问的基础功能，用「数量上限」作为分级杠杆（如标签 ≤ 20 个、订阅 Release ≤ 5 个仓库）。AI 工作流与 BYOK 配置在 2026-06-19 已改为 Pro-only，不再提供 App 内次数试用。

理由：基础整理功能需要降低上手门槛；AI 的产品心智应保持简单——Pro 解锁 Starcat 的智能工作流，模型费用由用户自己的 Provider 账户承担。

---

## 3. 完整功能盘点（截至 2026-06-15）

> 基线来源：`docs/功能实现总览.md` 进度仪表盘（P0 60/60 = 100%；P1 42/29 = 69%；P2 31/2 = 6%；总计 133 项 / 已完成 85 项 = 64%）。
>
...
### 5.9 CodeFlow 代码图谱（Pro）

| 功能 | 状态 | 理由 |
|---|---|---|
| CodeFlow 集成（GitHub ZIP 下载 + JSZip 解压 + 浏览器展示） | ✅ | 高级代码理解工具，与 AI 代码上下文同属增值能力 |
| 设置 → 集成 Tab 输出目录配置 / 用量统计 | ✅ | 配置可见；**生成 / 打开图谱**需 Pro |
...
| **AI 单仓摘要次数** | 0（免费版零 AI） | BYOK 后不做 App 内次数限制 | 2026-06-19 修订：AI 工作流本身归 Pro |
| **AI 标签推荐次数** | 0（免费版零 AI） | BYOK 后不做 App 内次数限制 | 同上 |
| **AI 对话（多轮 Chat）会话数** | 完全禁用 | 无限 + 磁盘持久化 + 多 session | |
| **README 翻译数量** | 完全禁用 | BYOK 后不做 App 内次数限制 | |
...
| **多 AI Provider 配置数** | 0 个 | 无限制 | BYOK 配置本身归 Pro |
| **JSON 导出/导入** | 无限制 | 无限制 | ✅ 一次性产物 |
| **Starred HTML / Markdown 导出** | 无限制 | 无限制 | ✅ 一次性产物 |
| **CloudKit 同步设备数** | 待定（见 §8） | 待定 | |

### 6.1 AI 试用策略（2026-06-19 修订）

- 不再提供 App 内 AI 次数试用；
- 免费用户不能配置 Provider / API Key，也不能触发 AI 摘要、标签、Chat、翻译、语义搜索等调用；
- BYOK 是 Pro 用户接入模型的方式，Pro 解锁 Starcat 的 AI 工作流层，模型 API 费用由用户自己的服务商账户承担；
- 如需试用，优先走 App Store Introductory Offer / Offer Code，让用户体验完整 Pro。
...
`docs/1-立项/需求分析.md` §7 已建议：上架首年 $19.99/年（标价 $29.99/年）。建议同时引入：
...
> 决策：**归 Pro**；**v1 首版不实现 CloudKit**。详见 `docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md` §2、§3。

**dong4j 选择**：选项 B（Pro）+ v1 整块延后。

- v1：无 iCloud entitlement、无同步代码；
- 后续：CloudKit 上线时设置页「启用 iCloud 同步」需 Pro 权益；
- 技术预研仍见 `docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md` §10（每 GitHub 账号独立 Zone）。

~~选项 A（免费）~~、~~选项 C（单向免费 / 双向 Pro）~~ — 未采用。

### ✅ D-2：AI 标签 / 摘要试用次数 — **已修订 2026-06-19**

> 决策：**0 次，免费版零 AI**。详见 `docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md` §2。

~~旧决策（各 3 次）~~、~~选项 B（10 次）~~ — 已被 2026-06-19 修订覆盖。

### ✅ D-3：免费版标签数量上限 — **已拍板 2026-06-18**

> 决策：**20 个**；第 21 个标签创建入口弹 Pro 付费墙。

~~选项 B（50 个）~~、~~选项 C（无限）~~ — 未采用。

### ✅ D-4：BYOK 用户是否仍需 Pro 订阅 — **已修订 2026-06-19**

> 决策：**需要 Pro**；免费用户不可配置 AI，Pro 解锁 BYOK 配置与 AI 工作流。

~~选项 B（BYOK 免费）~~ — 未采用。

### ✅ D-5：自动后台 AI 整理（HOM-126）是否提供"单次手动触发"给免费用户 — **已拍板 2026-06-18**

> 决策：v1 **完全 Pro only**；免费用户不开放手动批量整理。选项 B（免费单次 5 repo）留 v0.2 评估。

### ✅ D-6：Apple 生态集成（菜单栏 / Spotlight / Shortcuts / Widget）的免费/Pro 边界 — **已拍板 2026-06-18**

> 决策：v1 **暂不实施**；功能上线时再按“基础集成免费 / AI 唤起类 Pro”评估。

### ✅ D-7：AI Discovery（Show HN 抓取）的免费/Pro 边界 — **已拍板 2026-06-18**

后端是 starcat-weekly-api，已实现自动抓取 + LLM 分类。客户端**未开始**。

> 决策：客户端上线时 **列表浏览免费，AI 增强 Pro**。

### ✅ D-8：浏览器伴侣 Chrome 插件免费/Pro 边界 — **已拍板 2026-06-18**

> 决策：v1 **暂不实施**；插件上线时预埋 Pro 边界，基础 Star / 跳转能力免费，AI 总结与高级采集倾向 Pro。

---

## 9. 与现有文档的差异对账

### 9.1 与 `docs/1-立项/需求分析.md` §7 的差异
...
> 拍板结果见 **`docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md`**。以下为功能边界参考；StoreKit 代码实施需 dong4j 明确「开干」后启动。

### 10.1 短期（StoreKit 2 接入前 1 个月）

1. dong4j 在 §8 待决策项打钩 / 修订；
2. 根据决策更新 `docs/1-立项/功能清单.md` §7 和 `docs/1-立项/需求分析.md` §7 的定价方案章节；
...
1. 实现 StoreKit 2 + 订阅状态管理 + 14 天订阅试用 + 恢复购买；
...
1. 监控转化率（免费 → Pro 订阅试用 → Pro），调整 §6 数量上限；
...
- `docs/1-立项/功能清单.md` § 7 定价方案（待按本文档结论更新）
- `docs/1-立项/需求分析.md` § 7 定价策略建议（待按本文档结论更新）
- `docs/1-立项/概要设计.md` § 4.1 AI 调用模式（已明确"两种模式均需 Pro 订阅"）
- `docs/2-产品/需求讨论/正式方案/StoreKit订阅上架方案.md`（**v1 StoreKit 拍板 + 技术方案，优先阅读**）
- `docs/功能实现总览.md` § 5.3 订阅系统（StoreKit 2 接入项）
- `docs/2-产品/需求讨论/正式方案/AI代理API设计.md`（AI 后端代理设计）
- `docs/2-产品/需求讨论/正式方案/CloudKit数据同步设计.md`（CloudKit 同步设计）
