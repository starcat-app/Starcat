...
`feed`、`most-popular`、`new-releases` 继续保留为服务端分页读取与诊断能力;Starcat 主链路使用 `bulk` 落本地缓存后本地筛选、排序、分页。`trending` endpoint 可先实现但不接客户端,用于与旧 `trending-api` 对比。
...
- `trending.total` 与 `trending.languages[]`: 仅作为 discovery 新趋势候选数据质量对比。Starcat 当前趋势 UI 仍优先使用 `starcat-trending-api` 的语言聚合。
...
- `discovery_summary_modes`: 保存 4 个模式总量。
