...
Starcat 客户端新增 discovery 本地缓存,与现有 Trending 的“快速首屏 + 远端刷新”体验对齐:
...
sequenceDiagram
    ...
    UI->>Repo: load mode/filter/page
    Repo->>DB: read cached page/summary
    DB-->>Repo: cached snapshot if exists
    Repo-->>UI: render cached data first
    Repo->>API: fetch fresh page/summary
    API-->>Repo: fresh snapshot
    Repo->>DB: replace cache bucket
    Repo-->>UI: render fresh data
```

缓存边界:

- `discovery_list_pages`: 保存分页 meta。
- `discovery_list_items`: 保存公开仓库卡片快照。
