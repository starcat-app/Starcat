...
| 发版工程 | `scripts/release-store.sh` + git tag 驱动版本号 **已就绪** |
...
- [ ] **隐私政策 HTTPS 托管** — App Store 版关于页已链 `https://dong4j.app/starcat/privacy`，需确认页面真实可访问且与 App 实际数据行为一致
- [ ] **匿名遥测披露同步** — 隐私政策 / App Store Connect App Privacy 需写清 Aptabase opt-in、Usage Data / Diagnostics、Not Used to Track You
- [ ] **EULA / 用户协议** — App Store 版关于页已链 `https://dong4j.app/starcat/eula`，同上
- [ ] **支持页 / 联系邮箱** — App Store 版关于页链 `https://dong4j.app/starcat/support`、`mailto:dong4j@gmail.com`
...
- [ ] **发版命令演练**（可选 dry-run）：`./scripts/release-store.sh vX.Y.Z --dry-run`
...
- 发版 SOP：`docs/6-发版与上架/SOP-发版流程.md` + `scripts/release-store.sh`
...
flowchart TD
  ...
  K --> L[push → release-store.sh vX.Y.Z]
```
...
4. **`dong4j.app/starcat/privacy` / `eula`** — App Store 合规页面是否已上线且与 App 行为一致？
...
| `docs/6-发版与上架/SOP-发版流程.md` | tag / build / DMG / release-store.sh |
| `docs/6-发版与上架/SOP-App-Store-首次上架流程.md` | 第一次 Mac App Store 上架后台 / TestFlight / 审核全流程 |
