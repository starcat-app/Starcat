...
> ⚠️ **数据库 schema 变更必须追加新的 `registerVN` 迁移（`ALTER TABLE` / `CREATE TABLE`），禁止直接改已落地的 `v1-initial` 建表 SQL，禁止要求用户删库重建。**
> ⚠️ **应用层仍禁止堆叠无意义的「兼容旧 API / 双写旧字段 / 永久双轨」——废弃路径用迁移删掉或一次性替换，不要长期保留两套接口。**
> ⚠️ **看到代码层旧路径不再需要 → 直接删；数据库字段废弃 → 追加迁移处理，不要只改建表语句假装已上线用户会跟着变。**
