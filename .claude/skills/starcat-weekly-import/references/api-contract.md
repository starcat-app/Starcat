...
- Base URL：环境变量 `STARCAT_WEEKLY_BASE_URL`，例如 `https://weekly-api.example.com`。
- Admin Key：环境变量 `STARCAT_WEEKLY_ADMIN_KEY`。
...
响应为 `202 Accepted`，`data.batch_id` 是后续查询依据。同一个 `idempotency_key` 重放时返回原批次，不会重复建批。
