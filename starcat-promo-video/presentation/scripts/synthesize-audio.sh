...
while IFS= read -r row; do
  ...
  # Skip only non-empty files. A failed TTS call can leave a 0-byte mp3
  # (edge-tts creates the dest then errors), and treating that as "exists"
  # would permanently skip the segment on rerun.
