...
struct ShareCardContent: View {
    ...
    /// 底部品牌注脚收敛到右下角：能识别来源，但不与用户资料争夺视觉焦点。
    @ViewBuilder
    private var footerBar: some View {
        HStack {
            Spacer()
            Text(verbatim: "Starcat · Open source")
    ...
    /// ID 卡底部注脚：Generated by Starcat · 当前渠道官网域名
    @ViewBuilder
    private var idCardFooter: some View {
        HStack {
            Text("sharecard.card.generatedBy")
                ...
                .tracking(0.5)
            Spacer()
            Text(AppWebsiteLinks.currentDisplayHost)
                .font(.system(size: 9, weight: .medium, design: .monospaced))
                .foregroundStyle(.green)
        }
    }
