...
//  交错返回。本类型严格按 provider 给出的 choice/index 聚合，保留参数原文，完成后
//  再交给 Agent Runtime 做 JSON 解码和 schema 校验。
//
...
struct AIChatToolCallAccumulator: Sendable {
    ...
    private var callsByIndex: [CallKey: PartialCall] = [:]

    var isEmpty: Bool { callsByIndex.isEmpty }

    mutating func append(_ delta: AIChatToolCallDelta) {
        let key = CallKey(choiceIndex: delta.choiceIndex, callIndex: delta.index)
        var partial = callsByIndex[key] ?? PartialCall()
        if let id = delta.id, !id.isEmpty {
            // OpenAI 协议把 id 视为完整值而非文本 delta；部分兼容服务会在多个 chunk
            // 重复发送同一个 id，覆盖可避免拼成 `call-1call-1`。
            if !partial.id.isEmpty, partial.id != id {
                // 同一 choice/index 出现新 callID 表示 Provider 重用了槽位，不能把两次
                // 调用的 name/arguments 拼在一起。
                partial = PartialCall()
            }
            partial.id = id
        }
    ...
    func completedCalls(
        choiceIndex: Int = 0,
        idFactory: @Sendable () -> String = { UUID().uuidString }
    ) -> [AIChatToolCall] {
        callsByIndex.keys
            .filter { $0.choiceIndex == choiceIndex }
            .sorted { $0.callIndex < $1.callIndex }
            .compactMap { key in
            guard let partial = callsByIndex[key] else { return nil }
