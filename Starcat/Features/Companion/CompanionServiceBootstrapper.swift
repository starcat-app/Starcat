...
enum CompanionServiceBootstrapper {
    ...
    static func startFromStoredConfiguration() {
        #if DEBUG
    ...
    static func apply(configuration: CompanionConfiguration) {
        #if DEBUG
        guard !TestEnvironment.isRunning, DebugFlags.companionLocalServer, configuration.isEnabled else {
            stop()
            return
        }
        let nextServer = CompanionLocalServer(configuration: configuration)
        server?.stop()
        server = nextServer
        nextServer.start()
        #else
