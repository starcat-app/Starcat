...
final class TagManagementViewModel {
    ...
    func loadAll() async {
        ...
        do {
            ...
            errorMessage = String(format: String.l10n("tagManagement.error.loadFailedFormat"), error.localizedDescription)
        }
    }
    ...
    func create(name: String, color: String?, icon: String?) async -> Bool {
        ...
        guard !trimmed.isEmpty else {
            errorMessage = String.l10n("tagManagement.error.emptyName")
            return false
        }
        do {
            // UNIQUE 唯一性先查，UI 友好（避免直接抛 GRDB 错）
            if try await tagRepository.findByName(trimmed) != nil {
                errorMessage = String(format: String.l10n("tagManagement.error.duplicateNameFormat"), trimmed)
                return false
            }
    ...
    func update(_ tag: Tag, name: String, color: String?, icon: String?) async -> Bool {
        ...
        do {
            ...
            errorMessage = String(format: String.l10n("tagManagement.error.updateFailedFormat"), error.localizedDescription)
            return false
        }
    }
    ...
    func delete(ids: Set<String>) async {
        ...
        do {
            ...
            errorMessage = String(format: String.l10n("tagManagement.error.deleteFailedFormat"), error.localizedDescription)
        }
    }
    ...
    func merge(sources: Set<String>, into target: String) async {
        ...
        do {
            ...
            errorMessage = String(format: String.l10n("tagManagement.error.mergeFailedFormat"), error.localizedDescription)
        }
    }
