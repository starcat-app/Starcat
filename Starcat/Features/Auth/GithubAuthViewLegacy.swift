//
//  GithubAuthViewLegacy.swift
//  Starcat
//
//  ⚠️ Legacy fallback：原 `GithubAuthView` 的第一版实现，保留作为回滚 / 对比参考。
//  生产路径已切到新的 `GithubAuthView`（V2 视觉升级版），见 `GithubAuthView.swift`。
//
//  保留原因（dong4j 2026-06-03 决策）：
//  - 新版本上线后允许快速回退（直接把 ContentView 里 `GithubAuthView()` 换回 `GithubAuthViewLegacy()`）
//  - 历史代码风格 / 状态分支处理对比参考
//
//  本文件不应被生产代码引用；如果一段时间后新版本稳定，可整文件删除。
//
//  原 UI 状态：
//  - 未登录态：展示 GitHub 图标 + "使用 GitHub 登录"按钮
...
struct GithubAuthViewLegacy: View {
