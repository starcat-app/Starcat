...
final class StatusBarController: NSObject {
    ...
    private static func makeStatusIcon() -> NSImage {
        ...
        head.move(to: NSPoint(x: 4.5, y: 5.2))
        head.line(to: NSPoint(x: 4.5, y: 12.2))
        head.line(to: NSPoint(x: 7.0, y: 14.8))
        head.line(to: NSPoint(x: 8.7, y: 12.8))
        head.line(to: NSPoint(x: 11.3, y: 12.8))
        head.line(to: NSPoint(x: 13.0, y: 14.8))
        head.line(to: NSPoint(x: 15.5, y: 12.2))
        head.line(to: NSPoint(x: 15.5, y: 5.2))
        head.close()
        head.lineWidth = 1.5
        head.stroke()

        // 中心星点保留 Starcat 识别度；尺寸控制在 16pt 内，避免菜单栏缩放后糊成一团。
        let star = NSBezierPath()
        star.move(to: NSPoint(x: 10.0, y: 10.7))
        star.line(to: NSPoint(x: 10.6, y: 9.4))
        star.line(to: NSPoint(x: 12.0, y: 9.2))
        star.line(to: NSPoint(x: 10.9, y: 8.3))
        star.line(to: NSPoint(x: 11.2, y: 6.9))
        star.line(to: NSPoint(x: 10.0, y: 7.6))
        star.line(to: NSPoint(x: 8.8, y: 6.9))
        star.line(to: NSPoint(x: 9.1, y: 8.3))
        star.line(to: NSPoint(x: 8.0, y: 9.2))
        star.line(to: NSPoint(x: 9.4, y: 9.4))
        star.close()
        star.fill()
