...
/// 这里保留窗口级单一状态，确保 Header 按钮与系统分栏始终读取同一份可见性。
@MainActor
@Observable
final class WorkspaceChromeState {
    ...
    /// Inspector API 使用“是否展示”，Header 仍使用“是否折叠”表达按钮状态；
...
struct WorkspaceTitlebarControls: View {
    ...
    var body: some View {
        platformControlGroup
            // 按钮数可变（Agent 3 / RAG 4），用 fixedSize 避免 Header 压缩时裁掉末尾按钮。
